    ...
    dir('scripts/trigger_jenkins_jobs') {
        withCredentials([usernamePassword(credentialsId: defaultParameters.jenkins.credentials_id, usernameVariable: 'JENKINS_USER', passwordVariable: 'JENKINS_PASSWORD')]) {
            ...
            if (conf.triggerJobMode == 'auto') {
                ...
                sh(script: "export JENKINS_CRUMB=${jenkinsCrumb};./trigger_jenkins_jobs.py --git-directory ${resultsDir} --images-file ${resultsFile} --ci-jobfile ${conf.jobsCiFile} --auto")
            } else {
                sh(script: "export JENKINS_CRUMB=${jenkinsCrumb};./trigger_jenkins_jobs.py --ci-jobfile ${conf.jobsCiFile} --platform ${conf.platform} -image ${conf.image} --image-url ${conf.imageUrl}  --dry-run")
            }
        }
    }
}
