def pushImage(Map jobParams) {
    dir("caasp-vmware/tools") {
        timeout(120) {
            withEnv(["VC_USERNAME=${jobParams.credentials.vmware_username}", "VC_PASSWORD=${jobParams.credentials.vmware_password}"]) {
                sh(script: "set -o pipefail; export VC_HOST=${jobParams.platformEndpoint}; ./upload-image-vmware.sh ${jobParams.varFile} ${jobParams.imageSourceUrl} 2>&1 | tee ${WORKSPACE}/logs/caasp-vmware.log")
            }
        }
    }
}
